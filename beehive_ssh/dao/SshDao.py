...
from beehive_ssh.model import SshGroup, Base, SshNode, SshUser, SshKey, BaseSshEntity, \
    GroupNodes, KeyUsers, ExtendedSshUser
...
class SshDbManager(AbstractDbManager):
    ...
    def remove_node_from_group(self, node, group):
        ...
        if main_group_objid == group.objid:
            raise ModelError('Group %s is the main group of node %s. It can not be removed' %
                             (group.uuid, node.uuid), code=400)
        try:
    ...
    @query
    def get_paginated_users(self, node_id=None, tags=None, page=0, size=10, order='DESC', field='id',
                            username=None, user_id=None, *args, **kvargs):
        ...
        query.add_join('ssh_node', 't4', 't3.fk_node_id=t4.id', left=False, inner=True, outer=False)
        query.add_join('keys_users', 't6', 't6.fk_user_id=t3.id', left=True, inner=False, outer=True)
    ...
    def get_nodes_and_groups(self, users=['root', 'centos'], group=None, node=None, node_name=None):
        ...
        """
        session = self.get_session()
        data = session.query(SshNode, SshUser, SshKey, SshGroup). \
            filter(SshUser.node_id == SshNode.id). \
            filter(GroupNodes.fk_node_id == SshNode.id).\
            filter(GroupNodes.fk_group_id == SshGroup.id). \
            filter(KeyUsers.fk_user_id == SshUser.id). \
            filter(KeyUsers.fk_key_id == SshKey.id). \
